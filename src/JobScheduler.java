...
public class JobScheduler {
    public static void scheduleJobs(Job[] jobs, String outputFilePath) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputFilePath, true))) {
            writer.write("Basic Job Scheduler Output:\n<br>");
    ...
    public static void minHeapify(Job[] jobs, int n, int i) {
        ...
        if (left < n && jobs[left].processingTime > jobs[smallest].processingTime) {
            smallest = left;
        }

        // If right child is smaller than smallest so far
        if (right < n && jobs[right].processingTime > jobs[smallest].processingTime) {
            smallest = right;
        }
    ...
    public static Job[] readJobsFromFile(String inputFilePath) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(inputFilePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                jobs[count++] = new Job(id, processingTime);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
